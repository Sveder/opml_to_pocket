   ...
   if ( len(o) == 0 and o.xmlUrl ): # Element is a single RSS feed
      ...
      try:
         ...
         if hasattr(f.feed, "title"):
            _print(Fore.CYAN + Style.BRIGHT + "[INFO] Processing feed: " + Style.RESET_ALL + f.feed.title + Style.DIM + " [" + o.xmlUrl + "]" + Style.RESET_ALL)
         else:
            _print(Fore.CYAN + Style.BRIGHT + "[INFO] Processing feed: " + Style.RESET_ALL + Style.DIM + o.xmlUrl + Style.RESET_ALL)
         ...
         for i in f.entries:
            try:
               ...
               if ( latest_timestamp > config.min_time and latest_timestamp > last_check ): # Added since last scan
                  _print(Fore.CYAN + Style.BRIGHT + "[INFO] Submitting item to pocket: " + Style.RESET_ALL + i.title + Style.DIM + " [" + i.link + "]" + Style.RESET_ALL);

                  try:
                     p.add(i.link, title=i.title, time=latest_timestamp)
                     sql_new_count += 1
                  except timeouted_pocket.RateLimitException:
                     _print(Fore.RED + Style.BRIGHT + "[ERROR] Rate limit exceeded. " + Style.RESET_ALL + Style.DIM + "https://getpocket.com/developer/docs/rate-limits" + Style.RESET_ALL)
                     sql_status_string = "rate limit exceeded"
                     write_database()
                     exit(4)
                  except KeyboardInterrupt:
                     _print(Fore.YELLOW + Style.BRIGHT + "[WARNING] Interrupted by user." + Style.RESET_ALL)
                     sql_status_string = "interrupted"
                     write_database()
                     exit(130)
                  except:
                     if hasattr(i, "title"):
                        _print(Fore.YELLOW + Style.BRIGHT + "[WARNING] Could not post element to pocket: " + Style.RESET_ALL + Style.DIM + i.title)
                     else:
                        _print(Fore.YELLOW + Style.BRIGHT + "[WARNING] Could not post element to pocket:" + Style.RESET_ALL)
                        _print("  " + Style.DIM + str(i))
                     _print("  " + str(sys.exc_info()[0]))
                     _print("  -> " + str(sys.exc_info()[1]) + Style.RESET_ALL)
                     sql_status_string = "done with errors"
                     json_failed_items += jsonpickle.encode(i, unpicklable=False, make_refs=False) + ","

            except KeyboardInterrupt:
               _print(Fore.CYAN + Style.BRIGHT + "[INFO] Interrupted by user." + Style.RESET_ALL)
               sql_status_string = "interrupted"
               write_database()
               exit(130)
               ...
               if hasattr(i, "title"):
                  _print(Fore.YELLOW + Style.BRIGHT + "[WARNING] Invalid element: " + Style.RESET_ALL + Style.DIM + i.title)
               else:
                  _print(Fore.YELLOW + Style.BRIGHT + "[WARNING] Invalid element:" + Style.RESET_ALL)
                  _print("  " + Style.DIM + str(i))
               _print("  " + str(sys.exc_info()[0]))
               _print("  -> " + str(sys.exc_info()[1]) + Style.RESET_ALL)
               sql_status_string = "done with errors"
         ...
         _print(Fore.YELLOW + Style.BRIGHT + "[WARNING] Could not process feed: " + Style.RESET_ALL + Style.DIM + o.xmlUrl + Style.RESET_ALL)
         _print("  " + Style.DIM + str(sys.exc_info()[0]))
         _print("  -> " + str(sys.exc_info()[1]) + Style.RESET_ALL)
         sql_status_string = "done with errors"
      ...
      _print(Fore.CYAN + Style.BRIGHT + "[INFO] OPML Element is empty: " + Style.RESET_ALL + Style.DIM + str(o) + Style.RESET_ALL)
...
def write_database():
    ...
    try:
       ...
       db.commit()
       db.close()
       _print(Fore.CYAN + Style.BRIGHT + "[INFO] Database written." + Style.RESET_ALL)
    except:
       _print(Fore.RED + Style.BRIGHT + "[ERROR] Could not insert log entry to database!"  + Style.RESET_ALL)
       _print("  " + Style.DIM + str(sys.exc_info()[0]))
       _print("  -> " + str(sys.exc_info()[1])  + Style.RESET_ALL)
       exit(2)
...
try:
   ...
   _print(Fore.RED + Style.BRIGHT + "[ERROR] Could not connect to SQLite 3 database!" + Style.RESET_ALL)
   _print("  " + Style.DIM + str(sys.exc_info()[0]))
   _print("  -> " + str(sys.exc_info()[1]) + Style.RESET_ALL)
...
try:
   ...
   _print(Fore.RED + Style.BRIGHT + "[ERROR] Could not parse OPML file!" + Style.RESET_ALL)
   _print("  " + Style.DIM + str(sys.exc_info()[0]))
   _print("  -> " + str(sys.exc_info()[1]) + Style.RESET_ALL)
   sql_status_string = "failed"
   write_database()
